...
import { useRouter } from 'next/navigation'; // Next.js v14 에서는 next/router 대신 next/navigation 에서 useRouter 가져옴.
  ...
  const handleNicknameValidation = async () => {
    try {
      const response = await axios.get(
        'http://localhost:8080/event-api/event/validate-nickname',
        {
          params: { nickname },
        },
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:8080/event-api/event/join',
        {
          nickname,
